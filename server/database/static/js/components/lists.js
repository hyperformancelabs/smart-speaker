...
function getListById(listId) {
    return (cachedLists || []).find((list) => (list.list_id || list.id) === listId);
}

function getNoteById(listId, noteId) {
    return (getListById(listId)?.items || []).find((item) => (item.item_id || item.id) === noteId);
}
    ...
    const buildUI = (lists) => {
                ...
                html += `
                    <div class="list-card glass-panel" style="padding: 1.5rem; display: flex; flex-direction: column; gap: 1rem; height: 100%;">
                        <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid var(--border-glass); padding-bottom: 0.5rem;">
                            <h3 style="font-size: 1.2rem; font-weight: 600;">${escapeHtml(list.list_name)}</h3>
                            <div style="display: flex; gap: 0.25rem;">
                                <button class="btn-icon text-muted" onclick="window.editList('${lId}')" title="Sửa tên Danh Sách" style="width: 30px; height: 30px; font-size: 0.9rem;">
                        ...
                        html += `
                            <div class="note-item" style="display: flex; align-items: flex-start; justify-content: space-between; gap: 0.5rem; padding: 0.75rem; background: rgba(0,0,0,0.2); border-radius: var(--radius-sm);">
                                <label style="display: flex; gap: 0.8rem; cursor: pointer; align-items: flex-start; flex: 1;">
                                    ...
                                    <span style="font-size: 0.95rem; line-height: 1.4; ${note.completed ? 'text-decoration: line-through; opacity: 0.5;' : ''}">${escapeHtml(note.content)}</span>
                                </label>
                                <div style="display: flex; gap: 0.25rem; flex-shrink: 0;">
                                    <button class="btn-icon text-muted" onclick="window.editNote('${lId}', '${nId}')" title="Sửa Note" style="width: 24px; height: 24px; font-size: 0.8rem;">
                                        <i class="fa-solid fa-pen"></i>
                                    </button>
        ...
        if (editForm) {
            editForm.addEventListener('submit', async (e) => {
                ...
                try {
                    if (type === 'list') {
                        await ListAPI.renameList(listId, newContent);
                        showToast("Đã đổi tên danh sách", "success");
                    } else if (type === 'note') {
                        await ListAPI.updateNote(listId, noteId, newContent);
...
window.editList = async (listId, oldName) => {
    ...
    window.openEditModal('list', listId, null, list?.list_name || '');
};

window.editNote = async (listId, noteId) => {
    const note = getNoteById(listId, noteId);
    window.openEditModal('note', listId, noteId, note?.content || '');
};
