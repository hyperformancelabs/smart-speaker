...
export async function renderTimers(silent = false) {
  ...
  if (!silent) {
    content.innerHTML = `
      <div class="timer-container" style="max-width: 800px; margin: 0 auto; width: 100%; padding-bottom: 2rem;">
        ...
        <!-- Lịch sử -->
        <h4 style="color: var(--text-muted); margin-bottom: 1rem; font-size: 0.9rem; letter-spacing: 1px; text-transform: uppercase;">Recent Timers</h4>
  ...
  const buildList = (timers) => {
    ...
    listEl.innerHTML = sorted
      .map((t) => {
        return `
            <div style="display: flex; justify-content: space-between; align-items: center; padding: 1.2rem 0; border-bottom: 1px solid var(--border-glass); cursor: pointer;"
                 onmouseover="this.style.background='var(--bg-panel-hover)'"
                 onmouseout="this.style.background='transparent'"
                 onclick="window.quickStartFromHistory(${t.duration_seconds}, '${t.label || ''}')">
                <div style="font-weight: 600; font-size: 1.1rem; color: var(--text-main); margin-left: 0.5rem;">${t.label || "Hẹn giờ ngẫu nhiên"}</div>
                <div style="color: var(--text-muted); font-size: 1.1rem; margin-right: 0.5rem;">${formatTime(t.duration_seconds)}</div>
            </div>
        `;
      })
      .join("");
  ...
  window.quickStartFromHistory = (totalSec, label) => {
